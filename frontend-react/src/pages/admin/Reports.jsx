...
import { Banknote, Briefcase, Users, Trash2, Eraser, Package, AlertTriangle, X } from "lucide-react";
...
const Reports = () => {
  ...
  const handleClearBookings = () => {
     setConfirmation({
        ...
        message: `You are about to delete ALL ${completedBookings.length} completed bookings. This is irreversible.`,
        type: "danger",
        onConfirm: async () => {
             try {
                await Promise.all(completedBookings.map(b => deleteBooking(b.userID)));
                await fetchData();
             } catch (err) {
              ...
              <tbody className="divide-y divide-slate-100">
                {completedBookings.length > 0 ? (
                  completedBookings.map((b) => (
                    <tr key={b.id || b.userID} className="hover:bg-slate-50 group">
                      ...
                      <td className="px-4 py-3 text-right">
                          <button 
                            onClick={() => handleDeleteBooking(b.id || b.userID)}
